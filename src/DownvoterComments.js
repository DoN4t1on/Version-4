...
export const DownvoterComments = () => {
  ...
  const { Id } = useParams();
  const [allDownvoters, setAllDownvoters] = React.useState([]);

  useQuery('getDownvoterComments', () => userServices.commonGetService(`/post/getDownvoterListComments/${Id}`), {
    refetchOnWindowFocus: false,
    onError: (error) => {
      toast.error(ErrorService.uniformError(error));
    },
    onSuccess: (res) => {
      setAllDownvoters(res.data.data);
    },
  });
